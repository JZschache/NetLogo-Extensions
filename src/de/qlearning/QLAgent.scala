 ...
 */ 
case class QLAgent(experimenting: Double, qValuesMap: Map[String,QLAgent.QValue], nTotal: Double, 
                   lastChoice: String, choiceAlg: (Map[String,QLAgent.QValue], List[String], Double, RandomHelper) => String,
    ...
    def updated(alt:String, reward: Double) : QLAgent = {
      ...
      QLAgent(experimenting, qValuesMap.updated(alt, newQvalue), nTotal + 1.0, alt, choiceAlg, decrease, (if (decrease) decN + 1.0 else decN))
    }
